...
export class SmartMigrationBypass {
    private mappings = {
    ...
    async executeSmartMigration(): Promise<MigrationStats> {
        console.log('🧠 Démarrage migration intelligente (bypass FK)...\n');
        ...
        try {
            ...
            await this.analyzeExistingCommandes();

            // 3. Migrer SANS nettoyer (ajout seulement)
            await this.migrateAirtableCommandes(stats);
    ...
    private async analyzeExistingCommandes(): Promise<void> {
        console.log('📋 Analyse des commandes existantes...');
    ...
    private async migrateAirtableCommandes(stats: MigrationStats): Promise<void> {
        console.log('📦 Migration des commandes Airtable...');
    ...
    private async migrateCommandeRecord(record: any, stats: MigrationStats): Promise<void> {
        try {
            ...
            let numero = fields['NUMERO DE COMMANDE'];
            const existingWithSameNumero = await prisma.commande.findFirst({
                where: { numeroCommande: numero }
            });

            if (existingWithSameNumero) {
                numero = `${numero}_MIGRATED_${Date.now()}`;
                console.log(`🔄 Numéro modifié pour éviter conflit: ${numero}`);
            }
            ...
            const commandeData = {
                ...
                // Relations (assuming you use foreign keys directly, otherwise use connect)
                magasinId: magasinId,
                clientId: clientId,

                // Données client
                clientNom: fields['NOM DU CLIENT'] || '',
                clientPrenom: fields['PRENOM DU CLIENT'] || '',
                ...
                // Divers
                autresRemarques: fields['AUTRES REMARQUES'] || '',
            ...
            stats.success++;
            console.log(`✅ ${numero} migrée avec succès`);
...
// Script principal
async function main() {
    const migrator = new SmartMigrationBypass();

    try {
        await migrator.executeSmartMigration();
